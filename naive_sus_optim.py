###sbatch --time=8:00:00 --cpus-per-task=48 --mem=512G --wrap "python naive_sus_optim.py"
...
### trying out the "real" dataset to see if it is possible at least to run a single condition set

# z_frac = xr.open_dataset("z_frac.nc")

# z_score_threshold = 3.0
# frac_sp_threshold = 0.6

# condition = (z_frac['z_scores'] > z_score_threshold) & (z_frac['frac_sp'] > frac_sp_threshold)
# transformed = xr.where(condition, 1, 0)
# transformed_df = transformed.to_dataframe(name='transformed_values').unstack(level=-1)['transformed_values']
# result_dict = {}
# for index, row in transformed_df.iterrows():
#     columns_with_one = row[row == 1].index.tolist()
#     result_dict[index] = columns_with_one
# sets = run_naive_sus(result_dict)
