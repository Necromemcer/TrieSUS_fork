...
def solve_cover_set(input_sets):
    ...
    while early_stop == 0 and status == cp_model.OPTIMAL:
        current_solution = [set_name for set_name, var in set_vars.items() if solver.Value(var) == 1]
        optimal_solutions.append(current_solution)
        # Add a constraint to exclude the current solution to find new ones
        model.Add(sum(set_vars[set_name] for set_name in current_solution) <= len(current_solution) - 1)
        status = solver.Solve(model)
        if status != cp_model.OPTIMAL:
            break
        next_solution = [set_name for set_name, var in set_vars.items() if solver.Value(var) == 1]
